...
def test_min_group_count_happy(min_group_count):
    ...
    avc = AVC(df=DF, column=COLUMN, min_group_count=min_group_count).avc_df

    # don't consider _na and _other as they are insensitive to
    # the min_group_count
    if "_na" in avc.index:
        avc.drop("_na", inplace=True)
    if "_other" in avc.index:
        avc.drop("_other", inplace=True)
    ...
    if min_group_count <= avc["count"].max():
        assert avc["count"].min() >= min_group_count
    else:
...
def test_min_group_ratio_happy(min_group_ratio):
    ...
    avc = AVC(df=DF, column=COLUMN, min_group_ratio=min_group_ratio).avc_df
