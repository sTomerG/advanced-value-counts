...
def get_avc_df(
    ...
    # change the values of the main groups to '_other' if their ratio or minimal count is too small
    if groupby_col:
    ...
    if round_ratio:
        value_counts_df["subgroup_ratio"] = value_counts_df["subgroup_ratio"].round(
            round_ratio
        )
        if groupby_col:
            value_counts_df["ratio_vs_total"] = value_counts_df["ratio_vs_total"].round(
                round_ratio
            )
    ...
    if groupby_col:
        return (
            value_counts_df.groupby([value_counts_df.index, column]).sum().sort_index()
        )
...
def add_summary_statistics(
    ...
    """Adds summary statistic of each subgroup and main group of a grouped-by DataFrame
    ...
    if groupby_col:
        ...
        # select the grouped value counts series to be concatenatd to a single dataframe
        series_to_concat = [rel_series_grouped, count_series_grouped]
        keys = ("subgroup_ratio", "count")
    else:
        # select the ungrouped value counts series to be conconatenated to a single dataframe
        series_to_concat = [rel_series, count_series]
        keys = ("ratio", "count")

    # concatenate the relative and the absolute series to get a dataframe with both
    value_counts_df = pd.concat(
        series_to_concat, axis=1, keys=keys  # names of the columns
    )
    if groupby_col:

        # add the statistics of the ungrouped value counts, and add them to a new group called '_all'
        for index in set(value_counts_df.index.get_level_values(column)):
            value_counts_df.loc[("_all", index), "subgroup_ratio"] = rel_series[index]
        ...
        # loop through the zero-level index, which defines the groups of the groupby column
        for groupby_index in set(value_counts_df.index.get_level_values(groupby_col)):

            # make sure the '_all' index, created above, isn't used
            if groupby_index != "_all":

                # total of a subgroup is always 1
                value_counts_df.loc[(groupby_index, "_total"), "subgroup_ratio"] = 1
...
def group_uncommon_values(
    ...
    """Changes column values to a specified string (from uncommon_group_name) based on minimal conditions of its counts and maxium condition of amount of unique values

    Args:
        df (pd.DataFrame): the relevant pandas DataFrame
        column (str): the column name where the values are in
        max_groups (int, optional): the maximum amount of different values that are allowed. Defaults to None.
        min_ratio (float, optional): the minimal ratio a value must have. Defaults to 0.
        min_count (int, optional): the minimal count a value must have. Defaults to 1.
        uncommon_group_name (str, optional): value to change the uncommon group names to. Defaults to '_other'

    Returns:
        pd.Series: the pd.Series of the column of the df, with possibly some values changed to the value of uncommon_group_name
    """

    # get the value counts of the column
    value_counts = df[column].value_counts()

    # determine the names of the groups that are allowed, based on if max_groups is set or not
    groups = (
    ...
    # get a truth value for when a value count is less than the minimal ratio or less than the minimal count
    conditions = (
    ...
    # replace labels with '_other' if the count (ratio) is less than the minimal count (ratio)
    return np.where(
        df[column].isin(value_counts[conditions].index), uncommon_group_name, df[column]
    )
...
def group_uncommon_subgroups(
    ...
    min_subgroup_count_condition = (value_counts_df["count"] < min_subgroup_count) | (
        value_counts_df["subgroup_ratio"] < min_subgroup_ratio
    )
    total_ratio_condition = value_counts_df["r_vs_total"] < min_subgroup_ratio_vs_total
    ...
    return np.where(
        (min_subgroup_count_condition | total_ratio_condition | max_subgroup_condition)
